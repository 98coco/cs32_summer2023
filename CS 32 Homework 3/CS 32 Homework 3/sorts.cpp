...
bool compareSensor(const Sensor& lhs, const Sensor& rhs)
{
      ...
      // come first.  Return true if lhs should come first.  Notice that
...
//        swap(s[0], s[1]);
    ...
    for (int l = 2; l <= s.size(); l++){ //where s.size is the size of our sensor vector
        Sensor sortMe = s[l-1]; //last elem of the part we are analyzing in the vector
        int i = l-2;
        while ( i >= 0 && comp(sortMe,s[i]) == true){
            s[i+1] = s[i];
            --i;
        }
        s[i+1] = sortMe;
      ...
      // destruction time.
    {
     ...
     vector<Sensor> auxSensors(sensors);  //
      ...
      //        with compareSensorPtr as the ordering relationship.
        ...
        //compareSensorPtr algorithm comp(x,y); x > y then x before y
        
        sort(sensorPtrs.begin(),sensorPtrs.end(),compareSensorPtr); //need to pass compareSensorPtr to see which pointer is greater
